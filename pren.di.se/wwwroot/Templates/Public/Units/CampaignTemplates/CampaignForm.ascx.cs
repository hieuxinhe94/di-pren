...
namespace PrenDiSe.Templates.Public.Units.CampaignTemplates
{
    ...
    {
        #region Properties
        public string GaTrackingFirstPart { get; set; }

        protected bool CompanyCoBothHidden { get { return !ShowAdressFields && !ShowCompany; }}

        public bool ShowAdressFields
        {
            get
            {
                if (!ShowPaymentFields && !IsOtherPayerForm)
                {
                    return false;
                }
                return !IsDigitalCampaign || IsOtherPayerForm || (RbInvoice.Checked ? true : false);
            }
        }

        public bool ShowPaymentFields
        {
            get { return !IsOtherPayerForm && (!IsValue(PropertyPrefix + "HideCard") || !IsValue(PropertyPrefix + "HideInvoice") || !IsValue(PropertyPrefix + "HideInvoiceOther") || !IsValue(PropertyPrefix + "HideAutopay") || !IsValue(PropertyPrefix + "HideAutoWithdrawal")); } 
        }

        protected bool ShowCompany
        {
            get
            {
                if (IsValue(PropertyPrefix + "HideCompany"))
                {
                    return false;
                }
                return !IsStudentCampaign;
            }
        }

        protected bool ShowPAR
        {
            get { return !IsValue(PropertyPrefix + "HidePAR"); }
        }

        protected string JavascriptOutput { get; set; }
        ...
        public bool FirstNameReadOnly { get; set; }
        public bool LastNameReadOnly { get; set; }
        ...
        {
            base.OnPreRender(e);
            if (IsValue("TermsConditionsPage"))
            {
                lnkTermsConditions.NavigateUrl = CurrentPage["TermsConditionsPage"] as string;
            }
            BtnBack.Text = !string.IsNullOrEmpty(BackButtonText) ? BackButtonText : "TILLBAKA";
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!IsOtherPayerForm)
            {
                JavascriptOutput = "VerifyAddressFieldsVisibility();";
            }

            //PAR-search not visible on student campaigns
            PhGetParInfo.Visible = !IsStudentCampaign && ShowPAR;
        ...
        protected void BtnSubmitFormOnClick(object sender, EventArgs e)
        {
            ...
            if (!ShowAdressFields || IsDigitalCampaign && SelectedPayMethod != PaymentMethod.TypeOfPaymentMethod.Invoice)
            { 
        ...
        private void SetUpPaymentOptions()
        {
            ...
            //Only one can be checked, this is the priority, low prio first. Different prio for digital campaigns where card is higher prio than invoice
            if (IsDigitalCampaign)
            {
                if (showInvoiceOther)
                    PaymethodChecker(checkInvoiceotherPayer: true);
                if (showAutopay)
                    PaymethodChecker(checkAutoPayment: true);
                if (showAutoWithdrawal)
                    PaymethodChecker(checkAutoWithdrawal: true);
                if (showInvoice)
                    PaymethodChecker(checkInvoice: true);
                if (showCard)
                    PaymethodChecker(true);
            }
            else
            {
                if (showInvoiceOther)
        ...
        private void SetupRadioButtonClick()
        {
            //If a digital campaign, add some click scripts that will show/hide adress fields
            if (IsDigitalCampaign)
            {
                RbCard.Attributes.Add("onclick", "javascript:FormHandler('.address',false);Track('" + GaTrackingFirstPart + "','ChoosePayment','" + PaymentMethod.TypeOfPaymentMethod.CreditCard + "');");
                RbInvoice.Attributes.Add("onclick", "javascript:FormHandler('.address',true);Track('" + GaTrackingFirstPart + "','ChoosePayment','"+ PaymentMethod.TypeOfPaymentMethod.Invoice +"');");
                RbInvoiceOtherPayer.Attributes.Add("onclick", "javascript:FormHandler('.address',false);Track('" + GaTrackingFirstPart + "','ChoosePayment','" + PaymentMethod.TypeOfPaymentMethod.InvoiceOtherPayer + "');");
                RbAutoPayment.Attributes.Add("onclick", "javascript:FormHandler('.address',false);Track('" + GaTrackingFirstPart + "','ChoosePayment','" + PaymentMethod.TypeOfPaymentMethod .DirectDebit+ "');");
                RbAutoWithdrawal.Attributes.Add("onclick", "javascript:FormHandler('.address',false);Track('" + GaTrackingFirstPart + "','ChoosePayment','" + PaymentMethod.TypeOfPaymentMethod.CreditCardAutowithdrawal + "');");
            }
            else
            {
        ...
        private void PaymethodChecker(bool checkCard = false, bool checkInvoiceotherPayer = false, bool checkInvoice = false, bool checkAutoPayment = false, bool checkAutoWithdrawal = false)
        {
