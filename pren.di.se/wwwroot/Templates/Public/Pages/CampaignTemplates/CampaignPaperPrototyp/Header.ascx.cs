...
namespace PrenDiSe.Templates.Public.Pages.CampaignTemplates.CampaignPaperPrototyp
{
    public partial class Header : EPiServer.UserControlBase
    {
        ...
        {
            string ogTitle = GetPropertyString("FaceBookTitle", CurrentPage, CurrentPage.PageName);
            CreateMetaTag("og:title", ogTitle, false);

            string ogImage = GetPropertyString("FaceBookImage", CurrentPage, CurrentPage["Campaign1TopImage"] as string);
            CreateMetaTag("og:image",  EPiServer.Configuration.Settings.Instance.SiteUrl.ToString() + ogImage, false);

            string ogDesc = GetPropertyString("FaceBookDescription", CurrentPage, CurrentPage.PageName);
            CreateMetaTag("og:description", ogDesc, false);
            ...
            if (Tools.Classes.EPiFunctions.SettingsPageSetting(CurrentPage, "ExcludeFromRobots") != null)
            {
                string[] pagetypesID = Tools.Classes.EPiFunctions.SettingsPageSetting(CurrentPage, "ExcludeFromRobots").ToString().Split(',');
        ...
        private static string GetPropertyString(string PropertyName, PageData pageData, string fallBackValue)
        {
            return pageData[PropertyName] as string ?? fallBackValue;
        }
