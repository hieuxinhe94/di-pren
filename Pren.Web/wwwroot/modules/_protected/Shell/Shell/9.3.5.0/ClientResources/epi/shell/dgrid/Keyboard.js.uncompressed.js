...
function (
    ...
    return declare([dgridKeyboard], {
        ...
        _createCustomKeyMap: function () {
            ...
            return {
                67: function (event) { // C
                    if (this._isCtrlOrMetaKeyPressed(event)) this._doCopy(event);
                },
                99: function (event) { // c
                    if (this._isCtrlOrMetaKeyPressed(event)) this._doCopy(event);
                },
                88: function (event) { // X
                    if (this._isCtrlOrMetaKeyPressed(event)) this._doCut(event);
                },
                120: function (event) { // x
                    if (this._isCtrlOrMetaKeyPressed(event)) this._doCut(event);
                },
                86: function (event) { // V
                    if (this._isCtrlOrMetaKeyPressed(event)) this._doPaste(event);
                },
