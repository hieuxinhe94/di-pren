...
namespace DagensIndustri.Tools.Jobs.Apsis
{
    ...
    public class MyJob
    {
        public static string Execute()
        {
            ...
            int numNewBounces = db.AddBouncesFromApsis();
            //Db.MssqlUpdateBouncedEmailsFromCirixAndSetForceRetry();           //email different in cirix: update email + forceRetry=true

            var custsRetry = db.GetRetryCusts();
            var custsNew = CirixDbHandler.CirixGetNewCusts();
            ...
            try
            {
                CirixDbHandler.FlagCustsInLetter(custsSendSuccess, "Y");
            }
            catch
            {
                //updated=Y, will be deleted from cirix LETTER table  
            }
