...
<script>
    // PLACE AFTER CURRENT INSIGHTS CODE
    ;(function() {
        window.burtApi = window.burtApi || [];
        window.burtApi.push(function() {

            var getApsis = function() {
                var subidMatch = window.top.document.location.search.match(/subid=(\d+)/);
                if(subidMatch && subidMatch.length === 2) {
                    return subidMatch[1];    
                }
                return null;
            };

            var apsisId = getApsis();
            if(apsisId && apsisId.length > 0) {
                window.burtApi.connect('burt.beacon', 'apsis-subid-dagensindustri-se', apsisId);
            }
        });
    })();
</script>