...
//    LinkButtonLoginLogout.Text = "Logga in";
//else
//    LinkButtonLoginLogout.Text = "Logga ut";

namespace DagensIndustri.Templates.Public.Units.Static
{
    public partial class TopLoginSso : EPiServer.UserControlBase
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (HttpContext.Current.User.Identity.IsAuthenticated)
            {
                PlaceHolderLoggedIn.Visible = true;
                PlaceHolderNotLoggedIn.Visible = false;
            }
            else
            {
                PlaceHolderLoggedIn.Visible = false;
                PlaceHolderNotLoggedIn.Visible = true;
            }


            if (!IsPostBack)
            {
                HyperLinkToLogInPage.NavigateUrl = EPiFunctions.GetLoginPageUrl(CurrentPage);
                HyperLinkToLogInBonDig.NavigateUrl = BonDigMisc.GetLoginUrl(BonDigMisc.BonDigLoginRetHandlerPath, HttpContext.Current.Request.Url.AbsoluteUri);
