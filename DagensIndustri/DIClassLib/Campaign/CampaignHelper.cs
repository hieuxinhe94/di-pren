...
namespace DIClassLib.Campaign
{
    public class CampaignHelper
    {
        ...
        public static string GetSubStartDate(int pageId, string campId)
        {
            try
            {
                var dsCamp = SubscriptionController.GetCampaign(campId);
                if (DbHelpMethods.DataSetHasRows(dsCamp))
                {
                    ...
                    {
                        new Logger(string.Format("CampaignHelper.GetSubStartDate() returned either no paperCode or productNo for campaignId {0}", campId));
                    }
                    return SubscriptionController.GetNextIssueDateIncDiRules(DateTime.Now, paperCode, productNo).ToString("yyyy-MM-dd");
                }
            }
            catch (Exception ex)
            {
                new Logger("Campaign.GetSubStartDate() failed CampId:" + campId + " on page with id: '" + pageId + "' failed", ex.ToString());
            }
